...
const SubMenuHeader = ({ title, onBack, theme }) => (
    <div className={`p-6 pt-10 md:pt-6 pb-2 flex items-center gap-4 ${theme.text}`}>
        <button type="button" onClick={onBack} className={`w-12 h-12 flex items-center justify-center -ml-3 rounded-full hover:bg-black/5 transition-colors`}><ChevronLeft size={24} strokeWidth={3} /></button>
...
const SwitchItem = ({ label, description, active, onToggle, theme, isDarkMode }) => (
    <div className={`w-full flex items-center justify-between p-4 rounded-2xl transition-colors group text-left hover:bg-black/5 ${isDarkMode ? 'hover:bg-white/10' : ''}`}>
        <div className="flex-1 cursor-pointer" onClick={onToggle}>
            <div className={`font-bold text-sm ${theme.text}`}>{label}</div>
            {description && <div className={`text-[10px] mt-0.5 ${theme.textMuted}`}>{description}</div>}
        </div>
        <div className="relative flex items-center justify-center w-14 h-8 overflow-visible z-[10] ml-2 mr-[-4px]">
            <LiquidGooToggle
                checked={active}
                onChange={onToggle}
    ...
    return (
        <div className="flex flex-col h-full animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5 scrollbar-hide">
                {activeTab === 'info' ? (
                    <>
                        <div className="space-y-2"><label className={`text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 ${theme.textMuted}`}><User size={12} /> Nombre Completo</label><LiquidGlassInput value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} isDarkMode={isDarkMode} className="!h-14 font-bold" /></div>
                        ...
                        <div className="space-y-2"><label className={`text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 ${theme.textMuted}`}><Mail size={12} /> Email</label><LiquidGlassInput value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} isDarkMode={isDarkMode} className="!h-14 font-bold" /></div>
                    </>
                ) : (
                    <>
                        ...
                        <div className="space-y-2"><label className={`text-[10px] font-bold uppercase tracking-wider ${theme.textMuted}`}>Contraseña Actual</label><LiquidGlassInput type={showPasswords.current ? "text" : "password"} value={securityData.current} onChange={e => setSecurityData({ ...securityData, current: e.target.value })} placeholder="Escribe tu clave actual" isDarkMode={isDarkMode} className="!h-14 font-bold" rightIcon={<button type="button" onClick={() => togglePass('current')} className={`flex items-center ${theme.textMuted} hover:${theme.text}`}>{showPasswords.current ? <EyeOff size={18} /> : <Eye size={18} />}</button>} /></div>
                        <div className="space-y-2"><label className={`text-[10px] font-bold uppercase tracking-wider ${theme.textMuted}`}>Nueva Contraseña</label><LiquidGlassInput type={showPasswords.new ? "text" : "password"} value={securityData.new} onChange={e => setSecurityData({ ...securityData, new: e.target.value })} placeholder="Nueva clave" isDarkMode={isDarkMode} className="!h-14 font-bold" rightIcon={<button type="button" onClick={() => togglePass('new')} className={`flex items-center ${theme.textMuted} hover:${theme.text}`}>{showPasswords.new ? <EyeOff size={18} /> : <Eye size={18} />}</button>} /></div>
                        <div className="space-y-2"><label className={`text-[10px] font-bold uppercase tracking-wider ${theme.textMuted}`}>Confirmar Nueva</label><LiquidGlassInput type={showPasswords.confirm ? "text" : "password"} value={securityData.confirm} onChange={e => setSecurityData({ ...securityData, confirm: e.target.value })} placeholder="Repetir clave" isDarkMode={isDarkMode} className="!h-14 font-bold" rightIcon={<button type="button" onClick={() => togglePass('confirm')} className={`flex items-center ${theme.textMuted} hover:${theme.text}`}>{showPasswords.confirm ? <EyeOff size={18} /> : <Eye size={18} />}</button>} /></div>
                    </>
                )}
            </div>
...
const ViewNotifications = ({ theme, isDarkMode }) => {
    ...
    return (
        <div className="px-6 pb-6 animate-in slide-in-from-right duration-300">
...
const ProfileMenu = ({ user, onClose, onLogout, isDarkMode, toggleTheme, theme, showToast, onUpdateUser, onViewImage, onOpenEditor, showTools, setShowTools, mapTheme, setMapTheme, starrySky, setStarrySky }) => {
                    ...
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-1 scrollbar-hide">
                            ...
                            <div className={`w-full flex items-center justify-between p-3.5 rounded-xl transition-all group mb-1 ${isDarkMode ? 'text-white hover:bg-white/10' : 'text-black hover:bg-black/5'}`}>
                                ...
                                <div className="relative flex items-center justify-center w-14 h-8 overflow-visible z-[100] mr-2">
                                    <LiquidGooToggle
                                        checked={showTools}
                        ...
                        <div className={`px-6 pb-6 pt-4 flex-shrink-0 ${isDarkMode ? 'bg-black/20' : 'bg-gray-50'}`}>
    ...
    return (
        <div onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} className={`absolute inset-0 md:inset-[4px] md:rounded-[44px] shadow-none border-0 z-50 flex flex-col overflow-hidden animate-in slide-in-from-right duration-300 ease-out ${isDarkMode ? 'bg-[#121214]/40 backdrop-blur-xl border-white/20' : 'bg-white/40 backdrop-blur-xl border-black/10'}`}>
            {renderContent()}
        </div>
    );
};
